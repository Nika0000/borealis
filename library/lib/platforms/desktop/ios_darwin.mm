...
- (void)setMotorAmplitude:(unsigned short)amplitude duration:(unsigned int)durationMs;
- (void)cleanup;
...
- (void)setMotorAmplitude:(unsigned short)amplitude duration:(unsigned int)durationMs API_AVAILABLE(ios(14.0), tvos(14.0))
{
    NSError* error;
    ...
    // Always create a new player with the specified duration
    if (_hapticPlayer != nil)
    {
        [_hapticPlayer cancelAndReturnError:nil];
        _hapticPlayer = nil;
    ...
    // We must initialize the intensity to the desired value
    CHHapticEventParameter* intensityParameter = [[CHHapticEventParameter alloc] initWithParameterID:CHHapticEventParameterIDHapticIntensity value:amplitude / 65535.0f];
    CHHapticEvent* hapticEvent                 = [[CHHapticEvent alloc] initWithEventType:CHHapticEventTypeHapticContinuous parameters:[NSArray arrayWithObject:intensityParameter] relativeTime:0 duration:duration];
...
{
    init_device_rumble();
    [contextForLowFreqMotor setMotorAmplitude:lowFreqMotor duration:durationMs];
    [contextForHighFreqMotor setMotorAmplitude:highFreqMotor duration:durationMs];
}
